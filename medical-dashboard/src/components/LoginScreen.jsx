      ...
      {digits.map((d, i) => {
        ...
        return (
          <button
            ...
            onClick={() => onDigit(d)}
            disabled={disabled}
            className="h-12 rounded-xl bg-gray-100 hover:bg-blue-100 hover:text-blue-700 font-semibold text-gray-800 text-lg transition-all active:scale-95 disabled:opacity-40"
          >
            {d}
          </button>
        )
...
export default function LoginScreen({ onLogin }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'linear-gradient(150deg, #15314A 0%, #0E1626 100%)' }}>
      <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 overflow-hidden">
        {/* Header */}
        <div className="pt-8 pb-4 px-6 text-center">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center shadow-lg mx-auto mb-3"
            style={{ background: 'linear-gradient(135deg, #4FA6DD 0%, #C8A24A 100%)' }}>
            <Heart size={22} className="text-white" fill="white" />
          </div>
          <div className="font-bold text-gray-800 text-2xl" style={{ fontFamily: "'Spectral', Georgia, serif" }}>Vitae</div>
          <div className="text-gray-400 text-xs mt-0.5">Cartella clinica personale</div>
        </div>
